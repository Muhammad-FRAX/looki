...
import type { LookupResponse } from '../../src/lookup/types.js';

function makeMockRedis(overrides: Record<string, ReturnType<typeof vi.fn>> = {}) {
  return {
...
describe('setCachedLookup', () => {
  ...
  it('stores a round-trippable JSON value', async () => {
    ...
    const mockSet = vi.fn().mockImplementation((_k: string, v: string) => {
      stored = v;
