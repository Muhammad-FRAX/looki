...
import { signAccessToken, signRefreshToken, verifyAccessToken, verifyRefreshToken, requireJwt, requireAdmin, } from '../../src/auth/jwt.js';
...
describe('requireJwt middleware', () => {
    ...
    it('calls next and sets req.user for a valid JWT', () => {
        ...
        expect(req.user).toEqual({ id: 'user-abc', role: 'user' });
    });
    it('propagates the admin role to req.user', () => {
        ...
        const next = vi.fn();
        requireJwt(req, res, next);
        expect(req.user).toEqual({ id: 'admin-xyz', role: 'admin' });
    });
});
